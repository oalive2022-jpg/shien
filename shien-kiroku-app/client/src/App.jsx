...
import {
  ...
  Utensils, Moon, Pill, ArrowLeft, Users, Lock, ClipboardList, BarChart3, ChevronLeft, ChevronRight, Activity, Stethoscope, ChevronDown, FileSpreadsheet, FileSearch, Target, HeartPulse,
} from "lucide-react";
import { api, getToken, setToken, downloadExport, downloadResidentsExcel } from "./api";
...
const RECORD_TYPES = {
  ...
  nursing: {
    key: "nursing", label: "看護記録", icon: Stethoscope, color: "#8A6BA1",
    fields: [
      { key: "subjective", label: "S", type: "textarea", main: true, alwaysShow: true },
      { key: "objective", label: "O", type: "textarea", main: true, alwaysShow: true },
      { key: "assessment", label: "A", type: "textarea", main: true, alwaysShow: true },
      { key: "plan", label: "P", type: "textarea", main: true, alwaysShow: true },
    ],
  },
  assessment: {
    key: "assessment", label: "アセスメントシート", icon: FileSearch, color: "#A16B4C", dateLabel: "作成日",
    fields: [
      { key: "author", label: "作成担当者", type: "text" },
      { key: "livingSituation", label: "生活状況", type: "textarea", main: true },
      { key: "mentalPhysicalStatus", label: "心身の状況", type: "textarea", main: true },
      { key: "familySituation", label: "家族の状況", type: "textarea", main: true },
      { key: "otherNotes", label: "その他特記事項", type: "textarea", main: true },
    ],
  },
  support_plan: {
    key: "support_plan", label: "支援計画", icon: Target, color: COLORS.primary, dateLabel: "計画作成日",
    autoAlertField: "alertDate", autoAlertMonths: 6,
    fields: [
      { key: "author", label: "計画作成担当者", type: "text" },
      { key: "alertDate", label: "アラート表示日", type: "date" },
      { key: "userRequest", label: "利用者本人の要望", type: "textarea", main: true },
      { key: "familyRequest", label: "家族等の要望", type: "textarea", main: true },
      { key: "overallPolicy", label: "総合的な援助の方針（支援内容）", type: "textarea", main: true },
      { key: "longTermGoal", label: "長期目標", type: "textarea", main: true },
      { key: "shortTermGoal", label: "短期目標・その他", type: "textarea", main: true },
    ],
  },
  nursing_plan: {
    key: "nursing_plan", label: "看護計画", icon: HeartPulse, color: "#8A6BA1", dateLabel: "計画作成日",
    fields: [
      { key: "author", label: "計画作成担当者", type: "text" },
      { key: "nursingProblem", label: "看護問題・ニーズ", type: "textarea", main: true },
      { key: "goal", label: "目標", type: "textarea", main: true },
      { key: "interventions", label: "具体的な看護内容", type: "textarea", main: true },
    ],
  },
};
const RECORD_TYPE_ORDER = ["support", "health", "nursing", "meal", "stay"];
const PLAN_TYPE_ORDER = ["assessment", "support_plan", "nursing_plan"];
...
function RecordForm({ schema, onAdd, currentStaff }) {
  ...
  const changeDate = (v) => {
    setDate(v);
    if (schema.autoAlertField && !alertTouched) {
      setValues((prev) => ({ ...prev, [schema.autoAlertField]: v ? addMonths(v, schema.autoAlertMonths) : "" }));
    }
  };
  const changeField = (key, v) => {
    if (key === schema.autoAlertField) setAlertTouched(true);
  ...
  const submit = async () => {
    if (!canSave) return;
    setSaving(true);
    try { await onAdd({ date, data: values }); setValues(emptyValues(schema)); setAlertTouched(false); }
  ...
  return (
    <div className="skn-card rounded-lg p-4 space-y-2">
      <div className="flex flex-wrap gap-2 items-center">
        <input type="date" className="skn-input rounded px-2 py-1.5 text-sm" value={date} onChange={(e) => changeDate(e.target.value)} />
        {schema.fields.filter((f) => !f.main).map((f) => (
          <div key={f.key} className="flex flex-col"><FieldInput field={f} value={values[f.key]} onChange={(v) => changeField(f.key, v)} /></div>
        ))}
      ...
      {schema.fields.filter((f) => f.main).map((f) => (
        <div key={f.key}>
          ...
          <FieldInput field={f} value={values[f.key]} onChange={(v) => changeField(f.key, v)} />
        </div>
      ))}
...
function DataSummary({ schema, data, fieldAuthors }) {
  ...
  return (
    <>
      ...
      {mains.map((f) => (f.alwaysShow || data[f.key]) && (
        <div key={f.key} className="mb-1.5 last:mb-0">
          ...
          <p className="text-sm whitespace-pre-wrap leading-relaxed" style={!data[f.key] ? { color: COLORS.inkSoft } : undefined}>
            {data[f.key] || "（未記入）"}
          </p>
        </div>
      ))}
  ...
  return (
    <div className="skn-card rounded-lg p-3.5 group">
      <div className="flex items-center justify-between mb-1.5 flex-wrap gap-y-1">
        <span className="text-xs" style={{ color: COLORS.inkSoft }}>{schema.dateLabel || "対象日"} {record.date}</span>
...
function groupRecordsByDate(records) {
  ...
  return Object.keys(map).sort((a, b) => b.localeCompare(a)).map((date) => ({ date, records: map[date] }));
}

function DateGroup({ date, records, schema, currentStaff, onEdit, onDelete, defaultOpen }) {
  ...
  return (
    <div className="mb-2.5">
      <button
        ...
        style={{ background: COLORS.lineSoft, color: COLORS.ink }}
      >
        {open ? <ChevronDown size={15} /> : <ChevronRight size={15} />}
        {date}（{WEEKDAY_JA[dow]}）
        <span className="text-xs font-normal" style={{ color: COLORS.inkSoft }}>{records.length}件</span>
      </button>
      {open && (
        <div className="mt-2 space-y-2.5 pl-1">
          {records.map((r) => (
            <RecordCard key={r.id} schema={schema} record={r} currentStaff={currentStaff} onEdit={(data) => onEdit(r.id, data)} onDelete={() => onDelete(r.id)} />
          ))}
        </div>
      )}
    </div>
...
function ExportPanel({ homes }) {
  ...
  const [msg, setMsg] = useState("");
  const [profileHomeId, setProfileHomeId] = useState("all");
  ...
  const doProfileExport = async () => {
    setProfileLoading(true);
    try { await downloadResidentsExcel(profileHomeId === "all" ? null : profileHomeId); }
    catch (e) { setProfileMsg(e.message); } finally { setProfileLoading(false); }
  };

  return (
    <div className="space-y-6">
      <div className="space-y-3">
        <p className="text-sm font-medium" style={{ color: COLORS.primaryDark }}>記録のCSV出力</p>
        <ErrorBanner message={msg} />
      ...
      <div className="space-y-3 pt-4" style={{ borderTop: `1px solid ${COLORS.lineSoft}` }}>
        <p className="text-sm font-medium" style={{ color: COLORS.primaryDark }}>利用者プロファイルのExcel出力</p>
        <p className="text-xs" style={{ color: COLORS.inkSoft }}>氏名・ふりがな・生年月日・連絡先・服薬情報を一覧にしたExcelファイル(.xlsx)を出力します。</p>
        <ErrorBanner message={profileMsg} />
        <label className="text-sm block"><span className="block mb-1" style={{ color: COLORS.inkSoft }}>ホーム</span>
          <select className="skn-input w-full rounded px-3 py-2 text-sm" value={profileHomeId} onChange={(e) => setProfileHomeId(e.target.value)}>
            <option value="all">すべてのホーム</option>
            {homes.map((h) => <option key={h.id} value={h.id}>{h.name}</option>)}
          </select></label>
        <div className="flex justify-end">
          <button disabled={profileLoading} onClick={doProfileExport} className="skn-btn-primary px-5 py-2.5 rounded-lg text-base flex items-center gap-1.5 disabled:opacity-40">
            {profileLoading ? <Loader2 size={14} className="animate-spin" /> : <FileSpreadsheet size={14} />} Excelをダウンロード
          </button>
        </div>
      </div>
    </div>
...
function MainApp({ account, homeId, homes, onChangeHome, onLogout, onOpenAdmin, onHomesChanged }) {
  ...
  const [planTab, setPlanTab] = useState("support_plan");
  ...
  const supportPlanAlert = useMemo(() => {
    const plans = recordsByType?.support_plan;
    if (!plans || plans.length === 0) return null;
    const latest = plans.slice().sort((a, b) => b.date.localeCompare(a.date))[0];
    const alertDate = latest.data?.alertDate;
    if (alertDate && alertDate < todayStr()) return alertDate;
    return null;
  }, [recordsByType]);
  ...
  const loadRecords = useCallback(() => {
    ...
    Promise.all([...RECORD_TYPE_ORDER, ...PLAN_TYPE_ORDER].map((t) => api.get(`/residents/${selectedId}/records/${t}`).then((r) => [t, r.records])))
        ...
        <aside className="w-72 flex-shrink-0 flex flex-col border-r" style={{ borderColor: COLORS.line, background: "#F0ECDF" }}>
          ...
          <div className="flex-1 overflow-y-auto skn-scroll px-2 pb-3 space-y-1">
            ...
            {filtered.map((u) => {
              ...
              return (
                <button key={u.id} onClick={() => { setSelectedId(u.id); setShowAddUser(false); setEditingBasic(false); setShowUsage(false); setShowPlans(false); }} className={`skn-tab w-full text-left rounded-r rounded-l-sm px-3 py-3 flex items-center gap-2.5 ${active ? "active" : ""}`} style={{ "--tabcolor": color }}>
          ...
          {!showAddUser && selected && !editingBasic && (
            <div className="max-w-2xl mx-auto space-y-5">
              <div className="skn-card rounded-lg p-5 relative">
                ...
                {supportPlanAlert && (
                  <div className="pl-3 mt-2">
                    <button
                      onClick={() => { setShowUsage(false); setShowPlans(true); setPlanTab("support_plan"); }}
                      className="text-xs px-2.5 py-1.5 rounded-lg inline-flex items-center gap-1.5"
                      style={{ background: COLORS.rose + "1A", color: COLORS.rose, border: `1px solid ${COLORS.rose}55` }}
                    >
                      <AlertTriangle size={13} /> 支援計画の見直し時期です（アラート表示日：{supportPlanAlert}）
                    </button>
                  </div>
                )}
              </div>
              ...
              {showUsage ? (
                ...
                <div>
                  <button onClick={() => setShowPlans(false)} className="text-sm px-3.5 py-2 rounded-full flex items-center gap-1 mb-3" style={{ color: COLORS.inkSoft, border: `1px solid ${COLORS.line}` }}>
                    <ArrowLeft size={12} /> 記録に戻る
                  </button>
                  <div className="flex gap-2 flex-wrap mb-4">
                    {PLAN_TYPE_ORDER.map((k) => {
                      const t = RECORD_TYPES[k]; const Icon = t.icon;
                      return (
                        <button key={k} onClick={() => setPlanTab(k)} className="text-sm px-3.5 py-2 rounded-lg flex items-center gap-1.5" style={{ background: planTab === k ? t.color : t.color + "1A", color: planTab === k ? "#fff" : t.color }}>
                          <Icon size={14} /> {t.label}
                          <span style={{ color: planTab === k ? "#ffffffcc" : COLORS.inkSoft }}>({recordsByType?.[k]?.length ?? "…"})</span>
                        </button>
                      );
                    })}
                  </div>
                  {(() => {
                    const t = RECORD_TYPES[planTab];
                    return (
                      <div>
                        <RecordForm schema={t} onAdd={(partial) => addRecord(planTab, partial)} currentStaff={account.displayName} />
                        <div className="mt-3 space-y-2.5">
                          {recordsLoading && <div className="flex justify-center py-6"><Loader2 className="animate-spin" size={16} /></div>}
                          {!recordsLoading && (recordsByType?.[planTab] || []).length === 0 && <p className="text-xs text-center py-6" style={{ color: COLORS.inkSoft }}>まだ記録がありません</p>}
                          {!recordsLoading && groupRecordsByDate(recordsByType?.[planTab]).map((g, idx) => (
                            <DateGroup key={g.date} date={g.date} records={g.records} schema={t} currentStaff={account.displayName} onEdit={editRecord} onDelete={deleteRecord} defaultOpen={idx === 0} />
                          ))}
                        </div>
                      </div>
                    );
              ...
              <div className="flex gap-2 flex-wrap items-center">
                ...
                <button onClick={() => setShowPlans(true)} className="text-sm px-3.5 py-2 rounded-full flex items-center gap-1" style={{ color: COLORS.inkSoft, border: `1px solid ${COLORS.line}` }}>
                  <Target size={12} /> 計画書
                </button>
                ...
                return (
                  <div key={k} ref={(el) => (sectionRefs.current[k] = el)} style={{ scrollMarginTop: "16px" }}>
                    ...
                    <div className="mt-3 space-y-2.5">
                      ...
                      {!recordsLoading && groupRecordsByDate(recordsByType?.[k]).map((g, idx) => (
                        <DateGroup key={g.date} date={g.date} records={g.records} schema={t} currentStaff={account.displayName} onEdit={editRecord} onDelete={deleteRecord} defaultOpen={idx === 0} />
                      ))}
                    </div>
                  </div>
