    ...
    <div className="col-span-2 space-y-2">
      ...
      {medications.map((m) => (
        <div key={m.id} className="grid grid-cols-[1fr_1fr_90px_28px] gap-2 items-center">
          ...
          <button onClick={() => remove(m.id)} className="p-1.5 rounded hover:bg-black/5" style={{ color: COLORS.rose }}><Trash2 size={14} /></button>
        </div>
      ))}
      <button onClick={add} className="text-xs px-2.5 py-1.5 rounded flex items-center gap-1" style={{ color: COLORS.primary, border: `1px solid ${COLORS.primary}55` }}><Plus size={12} /> 服薬を追加</button>
    </div>
  );
}
                  ...
                  <tr key={date} style={{ borderTop: `1px solid ${COLORS.lineSoft}` }}>
                    ...
                    {medications.map((m) => (
                      <td key={m.id} className="px-2 py-1.5 text-center">
                        <input type="checkbox" checked={!!day.checks[m.id]} onChange={() => toggleCheck(date, m.id)} />
                      </td>
                    ))}
                    <td className="px-2 py-1.5">
        ...
        <aside className="w-72 flex-shrink-0 flex flex-col border-r" style={{ borderColor: COLORS.line, background: "#F0ECDF" }}>
          ...
          <div className="flex-1 overflow-y-auto skn-scroll px-2 pb-3 space-y-1">
            ...
            {filtered.map((u) => {
              ...
              return (
                <button key={u.id} onClick={() => { setSelectedId(u.id); setShowAddUser(false); setEditingBasic(false); setShowUsage(false); }} className={`skn-tab w-full text-left rounded-r rounded-l-sm px-3 py-2.5 flex items-center gap-2.5 ${active ? "active" : ""}`} style={{ "--tabcolor": color }}>
        ...
        <main className="flex-1 overflow-y-auto skn-scroll p-6">
          ...
          {!showAddUser && selected && !editingBasic && (
            <div className="max-w-2xl mx-auto space-y-5">
              ...
              {showUsage ? (
                <div>
                  <button onClick={() => setShowUsage(false)} className="text-xs px-2.5 py-1.5 rounded-full flex items-center gap-1 mb-3" style={{ color: COLORS.inkSoft, border: `1px solid ${COLORS.line}` }}>
              ...
              <div className="flex gap-2 flex-wrap items-center">
                {RECORD_TYPE_ORDER.map((k) => {
                  ...
                  return (
                    <button key={k} onClick={() => scrollToSection(k)} className="text-xs px-2.5 py-1.5 rounded-full flex items-center gap-1" style={{ background: t.color + "1A", color: t.color, border: `1px solid ${t.color}55` }}>
                      <Icon size={12} /> {t.label}
                      <span style={{ color: COLORS.inkSoft }}>({recordsByType?.[k]?.length ?? "…"})</span>
                    </button>
                  );
                })}
                <button onClick={() => scrollToSection("medication")} className="text-xs px-2.5 py-1.5 rounded-full flex items-center gap-1" style={{ background: COLORS.rose + "1A", color: COLORS.rose, border: `1px solid ${COLORS.rose}55` }}>
                  <Pill size={12} /> 服薬記録
                </button>
                <button onClick={() => setShowUsage(true)} className="text-xs px-2.5 py-1.5 rounded-full flex items-center gap-1 ml-auto" style={{ color: COLORS.inkSoft, border: `1px solid ${COLORS.line}` }}>
