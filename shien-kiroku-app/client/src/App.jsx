...
const RECORD_TYPE_ORDER = ["support", "health", "meal", "stay", "medication"];
        ...
        <aside className="w-72 flex-shrink-0 flex flex-col border-r" style={{ borderColor: COLORS.line, background: "#F0ECDF" }}>
          ...
          <div className="flex-1 overflow-y-auto skn-scroll px-2 pb-3 space-y-1">
            ...
            {filtered.map((u) => {
              ...
              return (
                <button key={u.id} onClick={() => { setSelectedId(u.id); setShowAddUser(false); setEditingBasic(false); }} className={`skn-tab w-full text-left rounded-r rounded-l-sm px-3 py-2.5 flex items-center gap-2.5 ${active ? "active" : ""}`} style={{ "--tabcolor": color }}>
          ...
          {!showAddUser && selected && !editingBasic && (
            <div className="max-w-2xl mx-auto space-y-5">
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                <button onClick={() => scrollToSection("usage")} className="text-xs px-2.5 py-1.5 rounded-full flex items-center gap-1" style={{ background: COLORS.blue + "1A", color: COLORS.blue, border: `1px solid ${COLORS.blue}55` }}>
                  <ClipboardList size={12} /> 実績記録
                </button>
              </div>
              ...
              <div ref={(el) => (sectionRefs.current.usage = el)} style={{ scrollMarginTop: "16px" }}>
