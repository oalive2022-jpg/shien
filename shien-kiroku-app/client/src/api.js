...
export async function downloadResidentsExcel(homeId) {
  const token = getToken();
  const q = homeId ? `?homeId=${homeId}` : "";
  const res = await fetch(`/api/residents-export${q}`, { headers: { Authorization: `Bearer ${token}` } });
  if (!res.ok) throw new Error("出力に失敗しました");
  const blob = await res.blob();
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = "residents_profile.xlsx";
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  URL.revokeObjectURL(url);
