...
const RECORD_TYPES = ["support", "meal", "stay", "medication"];
...
app.post("/api/residents", auth, async (req, res) => {
  const { homeId, name, kana, birthDate, contact } = req.body || {};
  if (!homeId || !name) return res.status(400).json({ error: "氏名と所属ホームは必須です" });
  const { rows } = await pool.query(
    "insert into residents (home_id, name, kana, birth_date, contact) values ($1,$2,$3,$4,$5) returning *",
    [homeId, name, kana || "", birthDate || null, contact || ""]
  );
...
app.put("/api/residents/:id", auth, async (req, res) => {
  ...
  if (!homeId || !name) return res.status(400).json({ error: "氏名と所属ホームは必須です" });
  const { rows } = await pool.query(
    "update residents set home_id=$1, name=$2, kana=$3, birth_date=$4, contact=$5 where id=$6 returning *",
    [homeId, name, kana || "", birthDate || null, contact || "", req.params.id]
  );
...
app.get("/api/export", auth, async (req, res) => {
  ...
  const fieldKeys = {
    ...
    stay: [["bedtime", "就寝時刻"], ["waketime", "起床時刻"], ["condition", "夜間の様子"], ["notes", "特記事項"]],
