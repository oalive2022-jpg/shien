...
create table if not exists records (
  id uuid primary key default gen_random_uuid(),
  resident_id uuid not null references residents(id) on delete cascade,
  type text not null check (type in ('support', 'health', 'nursing', 'meal', 'stay', 'medication', 'assessment', 'support_plan', 'nursing_plan')),
  record_date date not null,
...
-- 既存環境向け：以前のバージョンで作成されたテーブルのCHECK制約を更新する（計画書系を追加）
do $$
begin
  ...
  alter table records add constraint records_type_check check (type in ('support', 'health', 'nursing', 'meal', 'stay', 'medication', 'assessment', 'support_plan', 'nursing_plan'));
end $$;
...
-- 既存環境向け：食費・実費カラムを追加
alter table usage_records add column if not exists costs jsonb not null default '{}';
