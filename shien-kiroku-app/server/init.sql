...
create table if not exists records (
  id uuid primary key default gen_random_uuid(),
  resident_id uuid not null references residents(id) on delete cascade,
  type text not null check (type in ('support', 'health', 'nursing', 'meal', 'stay', 'medication')),
  record_date date not null,
...
-- 既存環境向け：以前のバージョンで作成されたテーブルのCHECK制約を更新する（看護記録を追加）
do $$
begin
  ...
  alter table records add constraint records_type_check check (type in ('support', 'health', 'nursing', 'meal', 'stay', 'medication'));
end $$;
